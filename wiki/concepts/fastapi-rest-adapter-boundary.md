...
DMS v0.7 source 문서는 `error_descriptor()`와 `recommended_http_error()`를 transport-neutral 계약으로 정의한다. 현재 `rag-system-core` v0.5.0은 `dms-core>=0.10.0`을 요구하므로, 이 versioned DMS v0.7 helper의 현재 runtime export 여부는 별도로 검증해야 한다. Host adapter는 실제 DMS error class가 제공하는 canonical `code`, `category`, `retryable`을 읽어 access 403, validation 400, payload-too-large 413, not-found 404, conflict/deleted 409, idempotency-in-progress 425, storage/metadata/health 503 등의 status로 projection한다. [[dms-error-and-http-contract]] ^[raw/articles/dms-api-reference-v0.7.0.md] ^[raw/articles/docmesh-api-reference.md]
...
현재 repository의 `ragflow/api/health.py`는 `core.health_check()`를 호출하지만, v0.5.0 package API reference는 `RAGCore`에 health method가 없고 package health-check API도 없다고 명시한다. 따라서 readiness는 host-owned dependency check로 전환하거나 package와 host가 일치하는 버전을 선택해야 한다. liveness/readiness 경로와 외부 dependency를 어느 정도까지 required로 볼지는 서비스 운영 정책이다. ^[raw/articles/docmesh-api-reference.md]

`ragflow/runtime.py` 역시 v0.5.0 문서에 없는 `required`, `check_on_startup`, `parallel_healthchecks` 인자를 runtime plan과 Factory에 전달한다. 이 source 문서와 현재 host 코드의 차이는 [[docmesh-v0-5-host-compatibility]]에서 비교한다.
...
- v0.5.0 package와 현재 host의 health/runtime signature compatibility를 먼저 해결해야 한다.
